...
export const myOrders = () => async (dispatch) => {
  try {
    ...
    let link = `/api/v1/orders/my_orders`;
...
// Update Order --- Admin
export const updateOrderAction = (orderId, order) => async (dispatch) => {
  try {
    dispatch({ type: UPDATE_ORDER_REQUEST });
    let link = `/api/v1/admin/order/${orderId}`;
    const config = { headers: { "Content-Type": "application/json" } };
